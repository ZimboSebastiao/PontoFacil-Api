...
function listarEmpresas(res) {
  const sql = "SELECT * FROM empresas ORDER BY nome";

  conexao.query(sql, (erro, resultados) => {
    if (erro) {
      res.status(400).json({ error: erro.code });
    } else if (resultados.length === 0) {
      res.status(204).json({ message: "Nenhuma empresa encontrada" });
    } else {
      res.status(200).json(resultados);
    }
  });
...
export { ler, inserir, lerUm, atualizar, excluir, listarEmpresas };
